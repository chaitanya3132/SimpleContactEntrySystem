package contactentrysystem.demo.controller;

import contactentrysystem.demo.contacts.controller.ContactController;
...
public class ContactControllerTest {
    ...
    public void createContactSuccess() {
        when(contactService.createContact(TEST_CONTACT_1)).thenReturn(TEST_CONTACT_1);
        Contact response = contactController.createContact(TEST_CONTACT_1);
        assertEquals(response.getName(), TEST_NAME_1);
    }

    @Test
    public void getContactSuccess() {
        when(contactService.getContactById(1234L)).thenReturn(TEST_CONTACT_1);
        Contact response = contactController.getContact(1234L);
        assertEquals(response.getId().longValue(), 1234L);
    }

    @Test
    public void getAllContactsSuccess() {
        when(contactService.getContacts()).thenReturn(TEST_CONTACT_LIST);
        List<Contact> response = contactController.getAllContacts();
    ...
    public void getCallListSuccess() {
        when(contactService.getCallList()).thenReturn(TEST_CONTACT_LIST);
        List<CallList> response = contactController.getCallList();
        assertEquals(response.get(0).getName().getFirst(), "TEST_FIRST_NAME_1");
    }

    @Test
    public void updateContactSuccess() {
        when(contactController.getContact(5678L)).thenReturn(UPDATE_CONTACT);
        contactService.updateContactById(5678L, UPDATE_CONTACT);
        assertEquals(contactController.getContact(5678L).getName().getFirst(), UPDATE_CONTACT.getName().getFirst());
    }

    @Test
    public void deleteContactSuccess() {
        contactService.deleteContactById(5678L);
        assertNull(contactController.getContact(5678L));
    }
